...
//            btn[i].setIcon(icon);
            {
                try {
                    tutu = ImageIO.read(new File("C:\\Users\\24550\\Desktop\\拼图.png"));
                    width = tutu.getWidth() / 4;
                    height = tutu.getHeight() / 4;

                    icon = new ImageIcon(tutu.getSubimage(width * row, height * col, width, height));
                    row++;
                    if (row == 4) {
                        row = 0;
                        col++;
...
//            btn[i].setOpaque(false);
    ...
    public void btnStart_Click() {
        ...
        for (i = 1; i < 500; i++) {
            ...
            Icon icon = btn[j].getIcon();
            btn[j].setIcon(btn[k].getIcon());
            btn[k].setIcon(icon);
    ...
    void btn_Click(int index) {
        ...
        if (isNeighbor(blank, index)) {       //如果相邻
            ...
            Icon icon = btn[blank].getIcon();
            btn[blank].setIcon(btn[index].getIcon());
            btn[index].setIcon(icon);
